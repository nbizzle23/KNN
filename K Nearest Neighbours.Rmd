...
In order to find the most similar NBA players we will use the principle of Euclidean distance, simply measuring the straight-line distance between two players. 

First import data from the website [basketballreference.com](https://www.basketball-reference.com/)
...
Since we are trying to predict the total amount of point a player scored we will save the ‘tpts’ column separately.


## Standardizing Data

Now using the scale function in R will now standardize the rest of the PerGame dataset
...
head(Standardized.PerGame)

```

Lets do a quick check on variance of the ‘fg’ column to verify that the scaling worked.
...
Now will we created train and test split set to run the KNN algorithm. Using random sampling, from the  standardized.PerGame data frame, and then pick rows using the randomly shuffled values.

First will we remove all the rows that have a NA value for the KNN function will not work.
...
We will create training (70%) set and 30% test set now.
